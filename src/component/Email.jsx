        ...
        <fieldset>
          <label className={`${composition_label}`} htmlFor="email">Email address</label>
            <div className={`container__input ${composition_container_input}`}>
              <input className={`${composition_input} ${error && composition_error_input}`} type="email" id="email" name="email" placeholder="Email address" required autoComplete="email" value={email} onChange={(e) => setEmail(e.target.value)}/>
              {error && (
                <img className={`icon__error ${composition_icon_error}`} src="./images/icon-error.svg" alt="exclamation mark" loading="lazy" decoding="async"/>
              )}
              <button className={`arrow_container ${composition_button} transition-transform duration-300 ${error && 'transform translate-x-4'}`}>
